...
Принимает на вход
    список (в виде обычного js массива с объектами внутри) и
    функцию-селектор, выбирающую из каждого объекта определенное значение.

Возвращает объект, в котором
    ключ - это результат применения функции селектора к каждому объекту в массиве, а
...
export default function objectify(arrayOfObj, selectorFn){
    function reduceHandler(accum, elem){
        accum[selectorFn(elem)] = elem;
        return accum;
    }
    return arrayOfObj.reduce(reduceHandler,{})
}
